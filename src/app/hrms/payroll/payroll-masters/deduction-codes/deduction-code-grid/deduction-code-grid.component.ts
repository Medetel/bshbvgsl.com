...
@Component({
  selector: 'app-deduction-code-grid',
  templateUrl: './deduction-code-grid.component.html',
  styleUrls: ['./deduction-code-grid.component.css']
})
export class DeductionCodeGridComponent implements OnInit {
  ...
  GetAllAllowanceDeductionCode(itemsPerPage: number, pageNo: number) {
    ...
    this.data = this.userService.GetAllAllowanceDeductionCode(itemsPerPage, pageNo, 'D');
    this.data.subscribe(
      (response: any) => {
  ...
  onSearch(s, itemsPerPage: number, pageNo: number) {
    ...
    if (searchText == "" || searchText == null || searchCriteria == "" || searchCriteria == null)
      ...
      document.getElementById('loader-spinner').style.display = "block";
      this.userService.GetAllAllowanceDeductionCodeonSearch(itemsPerPage, pageNo, 'D', searchText, searchCriteria)
        .subscribe(
          (response: any) => {
            this.allowedlist = response;
            if (this.allowedlist.length > 0)
