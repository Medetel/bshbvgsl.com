...
@Component({
  selector: 'app-leave-sanction-grid',
  templateUrl: './leave-sanction-grid.component.html',
  styleUrls: ['./leave-sanction-grid.component.css']
})
export class LeaveSanctionGridComponent implements OnInit {
  title = "Leave Sanction";
  ...
  GetEmployeeLeaveDetails(itemsPerPage: number, pageNo: number) {
    ...
    this.data = this.userService.GetAllEmployeeSanctionLeaveDetails(itemsPerPage, pageNo, this.c.DI_Id);
    this.data.subscribe(
      (response: any) => {
        this.EmployeeLeaveDetails = response;
        if (this.EmployeeLeaveDetails.length > 0)
          // this.EmployeeLeaveDetails = this.EmployeeLeaveDetails.filter(a=>a.LEAVE_APP_STATUS == 4062 || a.LEAVE_APP_STATUS == 4064)          
  ...
  ChangeDivision(DI_Id) {
    ...
    document.getElementById('loader-spinner').style.display = "block";
    this.data = this.userService.GetAllEmployeeSanctionLeaveDetails(this.itemsPerPage, this.currentPage, DI_Id);
    this.data.subscribe(
      (response: any) => {
        this.EmployeeLeaveDetails = response;
  ...
  //   this.filter = false;
  //   //this.GetEmployeeLeaveDetails(this.itemsPerPage,this.currentPage)
  //   this.FilteredEmployeeLeaveDetails = this.EmployeeLeaveDetails.filter(a => a.EMP_DIVISION_ID == DI_Id && (a.LEAVE_APP_STATUS == 4062 || a.LEAVE_APP_STATUS == 4064))
  ...
  save(Remarks) {
    ...
    this.data.subscribe(
      (response: any) => {
        swal('Success!', 'Leave Sanctioned', 'success');
        $('#myModal').modal('hide');
