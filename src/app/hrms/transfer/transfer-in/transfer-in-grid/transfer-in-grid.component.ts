...
@Component({
  selector: 'app-transfer-in-grid',
  templateUrl: './transfer-in-grid.component.html',
  styleUrls: ['./transfer-in-grid.component.css']
})
export class TransferInGridComponent implements OnInit {
  ...
  ngOnInit() {
    this.GetAllReportingDetails(this.itemsPerPage, 1);
  }
  ...
  pageChanged(pageNumber: number) {
    if (this.isSearch)
      ...
      this.GetAllReportingDetails(this.itemsPerPage, pageNumber);
  }
  ...
  itemsPerPageChanged(itemsPerPage: number, pageNo: number) {
    if (this.isSearch)
      ...
      this.GetAllReportingDetails(this.itemsPerPage, pageNo);
  }

  GetAllReportingDetails(itemsPerPage: number, pageNo: number) {
    this.isSearch = false;
    document.getElementById('loader-spinner').style.display = "block";
    this.userService.GetAllReportingDetails(itemsPerPage, pageNo)
      .subscribe(
        (data: any) => {
          document.getElementById('loader-spinner').style.display = "none";
          this.Reportinglist = data.TransferReportingModels;
...
delete(transfer_id){
  swal({
      ...
      if (result.value) {
      this.data = this.userService.DeleteDepuation(transfer_id);
      this.data.subscribe(
       (response: any) => {
           this.GetAllReportingDetails(this.itemsPerPage,this.currentPage);          
       },(error) => {
        document.getElementById('loader-spinner').style.display = "none";
    ...
    let searchCriteria = this.s.SearchCriteria;
      ...
      if (searchText == "" || searchText == null || searchCriteria == "" || searchCriteria == null)
        ...
        this.userService.Searchtransferdetails(searchCriteria, searchText, itemsPerPage, pageNo)
          .subscribe(
            (data: any) => {
              this.Reportinglist = data.TransferRelievingModels;
