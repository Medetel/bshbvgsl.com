...
@Component({
  selector: 'app-training-details-grid',
  templateUrl: './training-details-grid.component.html',
  styleUrls: ['./training-details-grid.component.css']
})
export class TrainingDetailsGridComponent implements OnInit {

  title="View Employee Training Details";  
  s: Search;
  ...
  ngOnInit() {
    this.GetAllTrainingDetails(this.itemsPerPage, 1);
  }
  ...
  pageChanged(pageNumber: number) {
    if (this.isSearch)
      ...
      this.GetAllTrainingDetails(this.itemsPerPage, pageNumber);
  }
  ...
  itemsPerPageChanged(itemsPerPage: number, pageNo: number) {
    if (this.isSearch)
      ...
      this.GetAllTrainingDetails(this.itemsPerPage, pageNo);
  }
  ...
  GetAllTrainingDetails(itemsPerPage: number, pageNo: number) {
    ...
    this.userService.GetAllTrainingDetails(itemsPerPage, pageNo)
      .subscribe(
        (data: any) => {
          document.getElementById('loader-spinner').style.display = "none";
          this.Traineelist = data.TrainingModel;
...
delete(TRAINING_ID){
  swal({
      ...
      if (result.value) {
      this.data = this.userService.DeleteTRN(TRAINING_ID);
      this.data.subscribe(
       (response: any) => {
           this.GetAllTrainingDetails(this.itemsPerPage,this.currentPage);          
       },(error) => {
        document.getElementById('loader-spinner').style.display = "none";
