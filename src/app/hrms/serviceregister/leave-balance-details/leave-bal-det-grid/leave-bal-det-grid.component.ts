...
@Component({
  selector: 'app-leave-bal-det-grid',
  templateUrl: './leave-bal-det-grid.component.html',
  styleUrls: ['./leave-bal-det-grid.component.css']
})
export class LeaveBalDetGridComponent implements OnInit {
  ...
  GetEmployeeBasicDetails(itemsPerPage: number, pageNo: number){
    ...
    this.data = this.userService.GetAllLeaveBalanceDetails(itemsPerPage,pageNo);
    this.data.subscribe(
         (response: any) => {
          this.BasicEmployeeDetails= response.LeaveBalanceModels;    
