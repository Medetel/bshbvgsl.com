...
@Component({
  selector: 'app-deputation-out-grid',
  templateUrl: './deputation-out-grid.component.html',
  styleUrls: ['./deputation-out-grid.component.css']
})
export class DeputationOutGridComponent implements OnInit {
...
DEPUTATION_ID:number;
  ...
  GetAllDeputationDetails(itemsPerPage: number, pageNo: number) {
    ...
    this.userService.GetAllDeputationDetails(itemsPerPage, pageNo)
      .subscribe(
        (data: any) => {
          document.getElementById('loader-spinner').style.display = "none";
          this.Deputationlist = data.DeputationModel;
          this.totalItems = data.DeputationModel[0].TotalItems;
