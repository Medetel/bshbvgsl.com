...
export class ApplicantRegisterComponent implements OnInit {
  ...
  filedata: any;
  title = "Apply For Applicant Registration";
  ...
  pageChanged(pageNumber: number) {
    ...
    if (this.isSearch)
      ...
      this.GetAllNotificationList(this.itemsPerPage, pageNumber);
  }
  ...
  GetAllNotificationList(itemsPerPage: number, pageNo1: number) {
    this.isSearch = false;
    document.getElementById('loader-spinner').style.display = "block";
    this.userService.GetAllNotificationListforapplicant(itemsPerPage, pageNo1)
      .subscribe(
        (data: any) => {
          document.getElementById('loader-spinner').style.display = "none";
          this.notificationlist = data.Notificationlist;
