...
@Component({
  selector: 'app-landrecordlist',
  templateUrl: './landrecordlist.component.html',
  styleUrls: ['./landrecordlist.component.css']
})
export class LandrecordlistComponent implements OnInit {
  ...
  GetLandRecordbyId(LR_Id, PD_Id) {
    ...
    this.userService.GetLandRecordbyId(LR_Id)
      .subscribe(
        (data: any) => {
          this.Land = data;
          //  this.GetProjectDetailsforLR(PD_Id)
          //  this.LR.PD_Id=PD_Id;
          if (this.Land.LR_Tippani_Date != null)
            this.Land.LR_Tippani_Date = ((this.Land.LR_Tippani_Date).split('T'))[0];
          if (this.Land.LR_GO_Date != null)
            this.Land.LR_GO_Date = ((this.Land.LR_GO_Date).split('T'))[0];
          if (this.Land.LR_LandConversion_Date != null)
            this.Land.LR_LandConversion_Date = ((this.Land.LR_LandConversion_Date).split('T'))[0];

          if (this.Land.LR_GO_File != null)
            this.Land.LR_GO_File = data.LR_GO_File;

          if (this.Land.LR_LandConversion_File != null)
            this.Land.LR_LandConversion_File = data.LR_LandConversion_File;
  ...
  getUserIdsFirstWay($event) {
    ...
    if (userId.length > 2) {
      if ($event.timeStamp - this.lastkeydown1 > 200) {
        this.userList1 = this.searchFromArray(this.projectlist, userId);
      }
    }
  }
  ...
  GetProjectDetailsforLR(projectCode) {
    ...
    this.projectDetails = {};
    this.data = this.userService.GetProjectDetailsforLR(projectCode, 2);
    this.data.subscribe(
      (response: any) => {
        ...
        for (let k = 0; k < this.LandRecord.length; k++) {
          if (this.LandRecord[k].LR_ExtentinAcres != null || this.LandRecord[k].LR_ExtentinGuntas != null) {
            ...
            // this.total = (parseFloat(this.TotalAcres) + parseFloat(this.TotalGuntas)/40);
          }
        }
  ...
  update(landdetails: NgForm) {    
    ...
    if (this.Land.LR_GO_File != null && !this.Land.LR_GO_File.match("http"))
      this.Land.LR_GO_File = this.getGOFile();
    if (this.Land.LR_LandConversion_File != null && !this.Land.LR_LandConversion_File.match("http"))
      this.Land.LR_LandConversion_File = this.getLandFile();
    this.show = 'N';
  ...
  UploadAtlas(file: FileList) {
    ...
    if (uploadedFilename.match(regex) && (size <= 40000)) {
    ...
    if (uploadedFilename.match(regex) && (size <= 40000)) {
      ...
      x.subscribe(
        (response) => {         
          ...
          let i: any = document.getElementById('LR_GO_File');
          i.value = "";
          if (error.status == 400) {           
            swal('Warning!', error.error.Message, 'warning');
          }
  ...
  getGOFile() {
    let imagename = null;
    try {
      imagename = document.getElementById('LR_GO_File');
      return imagename.files[0].name;
    }
    catch (e) {
      return null;
    ...
    if (uploadedFilename.match(regex) && (size <= 40000)) {
      ...
      x.subscribe(
        (response) => {        
          ...
          let i: any = document.getElementById('LR_LandConversion_File');
          i.value = "";
          if (error.status == 400) {          
            swal('Warning!', error.error.Message, 'warning');
          }
  ...
  getLandFile() {
    let imagename = null;
    try {
      imagename = document.getElementById('LR_LandConversion_File');
      return imagename.files[0].name;
    }
    catch (e) {
      return null;
