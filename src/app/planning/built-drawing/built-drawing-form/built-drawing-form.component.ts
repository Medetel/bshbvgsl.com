...
@Component({
  selector: 'app-built-drawing-form',
  templateUrl: './built-drawing-form.component.html',
  styleUrls: ['./built-drawing-form.component.css']
})
export class BuiltDrawingFormComponent implements OnInit {
  ...
  update(form: NgForm) {
    ...
    if (!form.invalid) {
      ...
      form.value.LP_AsBuilt_Drawing = 1;
      ...
      this.userService.updateKhbDrawingDetails(form.value, LP_Id)
        .subscribe(
          (data) => {
            ...
            this.router.navigate(['/home/builtdrawing']);
          }, (error) => {
            document.getElementById('loader-spinner').style.display = "none";
            this.errorHandler.handleError(error);
          });
