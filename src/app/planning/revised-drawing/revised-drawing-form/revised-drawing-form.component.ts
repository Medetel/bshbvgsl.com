...
@Component({
  selector: 'app-revised-drawing-form',
  templateUrl: './revised-drawing-form.component.html',
  styleUrls: ['./revised-drawing-form.component.css']
})
export class RevisedDrawingFormComponent implements OnInit {
  formSubmitted: boolean;
  mode: any;
  PD_Id: any;
  data: any;
  title="Revised Built Drawing";
  formInvalid: boolean;
      ...
      .subscribe(
        (response: any) => {
          ...
          if (response.ApprovedDrawingModel != null) {
            ...
            if (this.Layout != null && this.Layout.LP_AsBuilt_Drawing==1) {
              swal('Warning!', "Do you want to revise the Drawing", 'warning');
            }
            this.getAreaTotal()
      ...
      this.userService.updateKhbDrawingDetails(form.value, LP_Id)
        .subscribe(
          (data) => {
            ...
            this.router.navigate(['/home/reviseddrawing']);
          }, (error) => {
            document.getElementById('loader-spinner').style.display = "none";
            this.errorHandler.handleError(error);
          });
