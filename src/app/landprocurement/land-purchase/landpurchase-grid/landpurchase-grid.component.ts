...
@Component({
  selector: 'app-landpurchase-grid',
  templateUrl: './landpurchase-grid.component.html',
  styleUrls: ['./landpurchase-grid.component.css']
})
export class LandpurchaseGridComponent implements OnInit {

  title="Land Purchase";
  itemsPerPage: number=5;
  currentPage: number=1;
  //LandPurchaseList : any = {};
  LandPurchaseList : any[] = [];  //chamged bdy saheb
  data : any = {};
  totalItems : number;
  s: Search;
  isSearch:boolean;
  ...
  GetAllLandPurchase(itemsPerPage: number, pageNo: number){
    ...
    this.data = this.userService.GetAllLandPurchase(itemsPerPage,pageNo);
    this.data.subscribe(
         (response: any) => {          
