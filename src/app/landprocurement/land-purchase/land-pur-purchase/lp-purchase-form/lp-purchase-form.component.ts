...
@Component({
  selector: 'app-lp-purchase-form',
  templateUrl: './lp-purchase-form.component.html',
  styleUrls: ['./lp-purchase-form.component.css']
})
export class LpPurchaseFormComponent implements OnInit {
  Agreementlst: any = [];
  pdid: any;
  Lod_Id: any;
  LandDetailslist: any = [];
      ...
      (response: any) => {
        ...
        for (let i = 0; i < this.Agreementlst.length; i++) {
          if (this.Agreementlst[i].LP_Id_PK == LP_Id) {
            ...
            this.Village = this.Agreementlist[i].PR_Village;
  ...
  UpdateLandPurchase(LandPurchaseForm: NgForm) {
    ...
    if (LandPurchaseForm.value.LP_Purfile_path != null && !LandPurchaseForm.value.LP_Purfile_path.match("http"))
      LandPurchaseForm.value.LP_Purfile_path = this.getimageUpload1();
    this.data = this.userService.UpdateLandPurchase(LandPurchaseForm.value, LandPurchaseForm.value.LP_Id_PK, 2);
    this.data.subscribe(
      (response) => {
        LandPurchaseForm.reset();
        LandPurchaseForm.resetForm();
        ...
        this.router.navigate(['/home/landpurchase/land-pur-agreement/land-pur-purchase']);
        document.getElementById('loader-spinner').style.display = "none";
      }, (error) => {
        document.getElementById('loader-spinner').style.display = "none";
        if (error.status == 401) {
  ...
  GetByIdInspection(IR_Id) {
    ...
    this.data.subscribe(
      (response: any) => {
        ...
        this.l.LP_Purchase_Date = ((this.l.LP_Purchase_Date).split('T'))[0];
    ...
    if (uploadedFilename.match(regex) && (size <= 5048)) {
      ...
      x.subscribe(
        (response) => {
          ...
          let i: any = document.getElementById('LP_Purfile_path');
          i.value = "";
          if (error.status == 400) {
            swal('Warning!', error.error.Message, 'warning');
          }
  ...
  getimageUpload1() {
    let imagename = null;
    try {
      imagename = document.getElementById('LP_Purfile_path');
