...
@Component({
  selector: 'app-jv-possession-grid',
  templateUrl: './jv-possession-grid.component.html',
  styleUrls: ['./jv-possession-grid.component.css']
})
export class JvPossessionGridComponent implements OnInit {
  ...
  GetAllLandPurchase(itemsPerPage: number, pageNo: number){
    ...
    this.data = this.userService.GetJVpossession(itemsPerPage,pageNo);
    this.data.subscribe(
         (response: any) => {          
          this.LandPurchaseList= response.JointVentureModels;    
  ...
  delete(LP_Id_PK){
    swal({
        ...
        if (result.value) {
        this.data = this.userService.DeleteJV(LP_Id_PK);
        this.data.subscribe(
         (response: any) => {
             this.GetAllLandPurchase(this.itemsPerPage,this.currentPage);          
         },       
