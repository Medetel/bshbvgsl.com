...
@Component({
  selector: 'app-jv-exceptence-grid',
  templateUrl: './jv-exceptence-grid.component.html',
  styleUrls: ['./jv-exceptence-grid.component.css']
})
export class JvExceptenceGridComponent implements OnInit {
  ...
  GetAllLandPurchase(itemsPerPage: number, pageNo: number) {
    ...
    this.data = this.userService.GetJVpurchase(itemsPerPage, pageNo);
    this.data.subscribe(
      (response: any) => {
