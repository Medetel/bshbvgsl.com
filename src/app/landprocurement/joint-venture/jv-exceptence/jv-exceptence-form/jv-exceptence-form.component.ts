...
@Component({
  selector: 'app-jv-exceptence-form',
  templateUrl: './jv-exceptence-form.component.html',
  styleUrls: ['./jv-exceptence-form.component.css']
})
export class JvExceptenceFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {      
      ...
      if(this.mode=='view'){
        this.title = "View JV Acceptance";
      }
      else if(this.mode=='edit'){
        this.title = "Edit JV Acceptance";
      }
    });
  ...
  // else{ 
    if (LandPurchaseForm.value.JV_Purfile_path != null && !LandPurchaseForm.value.JV_Purfile_path.match("http"))
    LandPurchaseForm.value.JV_Purfile_path = this.getimageUpload1();
      this.data = this.userService.UpdateJV(LandPurchaseForm.value,LandPurchaseForm.value.JV_Id_PK,2);    
      this.data.subscribe(
    (response) => {   
      ...
      this.router.navigate(['/home/joint-venture/jv-exceptence']) ;   
      document.getElementById('loader-spinner').style.display = "none";           
    }, (error) => {
      document.getElementById('loader-spinner').style.display = "none";
      if (error.status == 401) {
  ...
  GetByIdInspection(IR_Id){   
    ...
    this.data.subscribe(
      (response: any) => {
        ...
        this.l.JV_DPC_Date = ((this.l.JV_DPC_Date).split('T'))[0];
        this.l.JV_BA_Date = ((this.l.JV_BA_Date).split('T'))[0];
        this.l.JV_GO_Date = ((this.l.JV_GO_Date).split('T'))[0];
        this.l.JV_Purchase_Date = ((this.l.JV_Purchase_Date).split('T'))[0];
      ...
      if (uploadedFilename.match(regex) && (size <= 5048)) {
        ...
        x.subscribe(
          (response) => {           
            ...
            let i: any = document.getElementById('JV_Purfile_path');
            i.value = "";
            if (error.status == 400) {                
              swal('Warning!', error.error.Message, 'warning');
            }
    ...
    getimageUpload1() {
      let imagename = null;
      try {
        imagename = document.getElementById('JV_Purfile_path');
